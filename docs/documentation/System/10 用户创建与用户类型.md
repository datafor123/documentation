...
In **Datafor**, administrators can create new users and assign a **User Type** during the creation process. The selected user type determines the account's basic capabilities, not its complete access to resources or data.
...
|--------------------------|----------------------------------------------------|
| **Reader (View-Only)** | Can view granted reports, dashboards, and permitted data, but cannot create or edit content. |
| **Creator (Content Creator)** | Can create content and edit resources they own or have permission to edit; shared-resource ACLs and Data Security still apply. |
