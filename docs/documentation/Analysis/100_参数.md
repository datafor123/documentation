...
| **Suggested values** | Select **Any value**, **SQL**, or **List of values**. Date parameters use **Any value**. |
...
| **List/Dropdown** | **SQL** or **List of values** |
| **Button** | **SQL** or **List of values** |

Select the controller, bind its **Parameter** field, and configure its display options. The picker groups **Report parameters** and **Global parameters**. **New parameter** in this picker creates a Report Parameter.

Changing a controller value updates the current runtime value and refreshes affected query components. Save the report to persist the controller, its binding, and its configuration. A viewer's selection does not rewrite a Global Parameter's **Default value**.
...
| Tab visibility | See [Parameter-Driven Tab Switching](/documentation/Visualization/Parameter-Driven-Tab-Switching/). |
