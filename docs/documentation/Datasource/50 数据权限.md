...
To use one, open the value list of a condition and choose an entry under **System variables**. The value is shown with an *ƒ* marker so that it is not mistaken for a fixed text. Typing the token is also accepted; the older `${system.…}` spelling is converted to `#{system.…}`.
...
- **Effective condition** shows the token as stored, for example `("OWNER" = #{system.username})`. **Validate expression** runs the condition with your own account substituted. The server rejects unknown variables, tokens placed inside quotes, and the `${…}` spelling.
