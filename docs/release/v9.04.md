...
- **Free-text values and system variables (9.04).** A condition value can be typed directly, with the field's existing values offered as suggestions, or it can be a system variable. **Current user name** (`#{system.username}`) and **Current user's roles** (`#{system.business-roles-array}`) are bound to the signed-in user when the query runs, so one policy such as `OWNER = Current user name` covers a whole role. **Validate expression** substitutes your own account, **Test access** shows the condition resolved for the simulated subject, and the server rejects unknown variables or tokens placed inside quotes.
- **Hidden tables inside a join path (9.04).** When a **Table & column access** policy hides a whole table that an analysis model joins through, for example `product` between `sales_fact` and `product_class`, the model now keeps that join: dimensions reached through the hidden table stay queryable, while the hidden table's own fields and dimension remain unavailable. Previously the affected reports failed with a model association error for those users.
...
- **Units and display scale (9.04):** measure units are stored as language-neutral codes (ISO 4217 currencies, CLDR units, %, counts) and shown in each user's language; a new **Display scale** setting (K / M / B, or 万 / 亿 for Chinese readers) becomes the measure's default number format in reports. Metrics Library uses the same unit picker and exports both the code and the display label. [Units and Display Scale](https://help.datafor.com.cn/documentation/Model/Units-and-Display-Scale/)
...
[AI Assistant](https://help.datafor.com.cn/documentation/AI-Agent/AI-Chat/) · [AI Agent Overview and Roadmap](https://help.datafor.com.cn/documentation/AI-Agent/AI-Agent-Overview-and-Roadmap/)
